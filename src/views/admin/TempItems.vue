<template>
  <div class="temp-items-container">
    <div class="page-header">
      <div>
        <h1 class="page-title">{{ $t('tempItems.adminTitle') }}</h1>
        <p class="page-subtitle">{{ $t('tempItems.adminSubtitle') }}</p>
      </div>
    </div>

    <div class="stats-grid">
      <div class="stat-card glass-card">
        <div class="stat-icon bg-orange-100"><svg class="stat-svg-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg></div>
        <div><p class="stat-value">{{ stats.pending }}</p><p class="stat-label">{{ $t('tempItems.pending') }}</p></div>
      </div>
      <div class="stat-card glass-card">
        <div class="stat-icon bg-green-100"><svg class="stat-svg-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>
        <div><p class="stat-value">{{ stats.received }}</p><p class="stat-label">{{ $t('tempItems.received') }}</p></div>
      </div>
      <div class="stat-card glass-card">
        <div class="stat-icon bg-teal-100"><svg class="stat-svg-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg></div>
        <div><p class="stat-value">{{ stats.total }}</p><p class="stat-label">{{ $t('common.total') }}</p></div>
      </div>
    </div>

    <div class="filter-bar glass-card">
      <div class="filter-group">
        <div class="search-wrapper"><svg class="search-svg-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg><input v-model="filters.search" type="text" :placeholder="$t('common.search')" class="search-input" @input="loadItems" /></div>
        <select v-model="filters.status" class="filter-select" @change="loadItems"><option value="">{{ $t('tempItems.allStatus') }}</option><option value="pending">{{ $t('tempItems.pending') }}</option><option value="received">{{ $t('tempItems.received') }}</option></select>
        <button @click="resetFilters" class="reset-btn">{{ $t('common.reset') }}</button>
      </div>
    </div>

    <div class="table-container glass-card">
      <div class="table-header"><h3>{{ $t('tempItems.registeredItems') }}</h3><span class="record-count">{{ items.length }} {{ $t('common.records') }}</span></div>
      <div class="table-responsive">
        <table class="data-table">
          <thead><tr><th>{{ $t('tempItems.itemNumber') }}</th><th>{{ $t('tempItems.itemName') }}</th><th>{{ $t('tempItems.quantity') }}</th><th>{{ $t('sales.customer') }}</th><th>{{ $t('tempItems.registeredBy') }}</th><th>{{ $t('tempItems.registeredAt') }}</th><th>{{ $t('common.status') }}</th><th>{{ $t('common.actions') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td>{{ item.item_number }}</td><td>{{ item.item_name }}</td><td>{{ item.quantity }}</td>
              <td>{{ item.customer_name || '-' }}</td><td>{{ item.registered_by }}</td><td>{{ formatDate(item.registered_at) }}</td>
              <td><span :class="getStatusClass(item.status)" class="status-badge">{{ getStatusText(item.status) }}</span></td>
              <td class="actions-cell">
                <button @click="viewItem(item)" class="action-btn view" :title="$t('common.view')">👁️</button>
                <button v-if="item.status === 'pending'" @click="receiveItem(item)" class="action-btn receive" :title="$t('tempItems.markReceived')">✅</button>
                <button v-if="item.status === 'pending'" @click="cancelItem(item)" class="action-btn cancel" :title="$t('common.cancel')">❌</button>
              </td>
            </tr>
            <tr v-if="items.length === 0"><td colspan="8" class="empty-table">{{ $t('tempItems.noItems') }}</td></tr>
          </tbody>
        </table>
      </div>
    </div>
    ...
    <div v-if="showViewModal" class="modal-overlay" @click.self="closeViewModal">
      <div class="modal-container">
        <div class="modal-header"><h2>{{ $t('tempItems.itemDetails') }}</h2><button @click="closeViewModal" class="modal-close">×</button></div>
        <div class="view-details" v-if="selectedItem">
          ...
          <div class="detail-row"><label>{{ $t('sales.customer') }}:</label><span>{{ selectedItem.customer_name || '-' }} {{ selectedItem.customer_phone || '' }}</span></div>
...
const stats = computed(() => ({ total: items.value.length, pending: items.value.filter(i => i.status === 'pending').length, received: items.value.filter(i => i.status === 'received').length }))

const loadItems = async () => {
  try {
    const params = {}; if (filters.value.search) params.search = filters.value.search; if (filters.value.status) params.status = filters.value.status
    items.value = (await api.get('/temp-items', { params })).data
  } catch (e) {}
}

const receiveItem = async (item) => {
  if (confirm(t('tempItems.receiveConfirm', { name: item.item_name }))) {
    try { await api.put(`/temp-items/${item.id}/receive`); await loadItems(); alert(t('tempItems.markedReceived')) } catch (e) { alert(t('tempItems.updateFailed')) }
  }
}

const cancelItem = async (item) => {
  if (confirm(t('tempItems.cancelConfirm', { name: item.item_name }))) {
    try { await api.put(`/temp-items/${item.id}/cancel`); await loadItems(); alert(t('tempItems.cancelled')) } catch (e) { alert(t('tempItems.cancelFailed')) }
  }
}

const viewItem = (i) => { selectedItem.value = i; showViewModal.value = true }
...
<style scoped>
/* Same styles as salesman view, plus: */
.action-btn.receive { color: #4caf50; }
.action-btn.receive:hover { background: rgba(76, 175, 80, 0.1); }
